...
public class AwakenYogSothothCard extends VisTable implements AncientOneCard{
    ...
    public void init(AncientOneInfo ancientOneInfo) {
        ...
        add(topLeftPart).align(Align.left);
        Image splashImage = new Image(CustomAssetManager.getTexture("ancient_one/YOG_SOTHOTH_AWAKEN.png"));
        splashImage.setAlign(Align.bottom);
        splashImage.setScaling(Scaling.fit);
        add(splashImage).growX().align(Align.bottom).row();
        add(createPowerTable()).padTop(5).align(Align.left);
    ...
    private Table createEndGameTable(AncientOneInfo ancientOneInfo) {
        ...
        table.row();
        table.add(winLabel).padLeft(2).padTop(5).width(73);
        table.add(winValue).padTop(5).align(Align.left).width(200).padLeft(5);
    ...
    private Table createPowerTable() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            powerTable.add(new Container<>(actor).size(50).padLeft(10));
        }
