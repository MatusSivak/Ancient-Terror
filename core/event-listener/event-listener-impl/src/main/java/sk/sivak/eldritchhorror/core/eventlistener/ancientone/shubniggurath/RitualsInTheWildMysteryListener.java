...
public class RitualsInTheWildMysteryListener extends AbstractMysteryListener implements EventListener<AvailableEncounters> {

    private AvailableEncounters availableEncounters;
    private EncounterActiveMysteryListener encounterActiveMysteryListener;

    public RitualsInTheWildMysteryListener(MysteryCardInfo mysteryCardInfo) {
    ...
    public void register() {
        ...
        if (!GoogleServicesHolder.isTutorialPassed()) {
            ServicePlatform.get().getGameService().showCurrentMysteryCard(true, true);
        } else {
    ...
    private class EncounterActiveMysteryListener extends EventListenerImpl<MysteryEncounter> {
        ...
        public void onNotify(MysteryEncounter eventData) {
            ...
            question.setTitle("Determine the ritual's purpose.");
        ...
        private void onAnswer(Answer<Boolean, Object> answer) {
            ServicePlatform.get().getTestService().test(Stat.OBSERVATION, 0, JUST_ONE).subscribe(testData -> {
                ...
                onSuccessfulTest(testData);
            });
        }

        private void onSuccessfulTest(TestData testData) {
            Question<Object> question = new Question<>();
            question.setOptions(Collections.singletonList(new Question.Option<>("OK", Boolean.TRUE)));
            question.setTitle("You discovered the purpose of this pagan rite.");
