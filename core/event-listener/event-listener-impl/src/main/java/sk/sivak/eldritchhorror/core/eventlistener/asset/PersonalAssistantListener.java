 ...
 * @author msivak
 */
public class PersonalAssistantListener extends AbstractAssetListener<PersonalAssistantAsset> {

    private static final Logger logger = LogManager.getLogger(PersonalAssistantListener.class);
    ...
    private class RegisterUsableAssetListener extends AbstractAssetEventListener<TestData> {
        ...
        public RegisterUsableAssetListener() {
            super(PersonalAssistantListener.this);
        }

        @Override
        protected Runnable getEventAction(TestData input) {
            return () -> {
                if (input.getStat() != Stat.INFLUENCE) {
                    return;
                }
