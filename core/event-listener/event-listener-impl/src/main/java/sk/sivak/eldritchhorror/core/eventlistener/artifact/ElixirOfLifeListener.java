...
import sk.sivak.eldritchhorror.core.constants.artifact.ElixirOfLifeArtifact;
...
public class ElixirOfLifeListener extends AbstractArtifactListener<ElixirOfLifeArtifact> {

    private static final Logger logger = LogManager.getLogger(ElixirOfLifeListener.class);
    ...
    private class InitActionButtonListener extends EventListenerImpl<Void> {
        ...
        public String toString() {
            return "ElixirOfLifeListener - InitActionButton";
        }
    }
    ...
    private class ActionListener extends AbstractActionPhaseListener<ActionListener.ElixirOfLifeAction> {
        ...
        protected String getTexturePath() {
            return "card/artifact/ELIXIR_OF_LIFE.jpg";
        }
        ...
        @Override
        protected ElixirOfLifeAction createAction() {
            return new ElixirOfLifeAction();
        }
        ...
        protected class ElixirOfLifeAction extends AbstractActionPhaseAction implements CardInfoAware<ElixirOfLifeArtifact> {
            ...
            private void onCanSpend(SpendData spendData) {
                ...
                showCardRequest.setTitle("Test Will to discard Conditions and recover all Health.");
            ...
            private void afterTest(TestData testData) {
                ...
                question.setTitle("Spend 1 Sanity to recover all Health and discard Conditions?");
            ...
            private void onSpend(SpendData spendData) {
                ...
                ServicePlatform.get().getService().hold();
                spendData.pay();
                List<ConditionInfo> badConditions = new LinkedList<>();
                badConditions.addAll(ServicePlatform.get().getConditionsDeck().getCondition(investigatorId, ConditionTrait.ILLNESS));
                badConditions.addAll(ServicePlatform.get().getConditionsDeck().getCondition(investigatorId, ConditionTrait.INJURY));
                badConditions.addAll(ServicePlatform.get().getConditionsDeck().getCondition(investigatorId, ConditionTrait.MADNESS));
                for (ConditionInfo badCondition : badConditions) {
                    ShowCardRequest showCardRequest = new ShowCardRequest();
                    showCardRequest.setConditionInfo(badCondition);
                    showCardRequest.setHideType(HideType.DISCARD_ALWAYS);
                    showCardRequest.setTitle("Discard " + badCondition.getName());
                    showCardRequest.setDisplayAssetResponseType(DisplayAssetResponseType.OK);
                    Single<ShowCardResponse> displayAsset = ServicePlatform.get().getGameService().showCard(showCardRequest);
                    displayAsset.subscribe(response -> {
                        ServicePlatform.get().getService().discardConditionFromInvestigator(investigatorId, badCondition);
                    });
                }
            ...
            @Override
            public ElixirOfLifeArtifact getCardInfo() {
