...
public class BlueMythosCard4 implements MythosCardEventListener{

    private static final String RUMOR_CARD_ID = "LostKnowledge";
    ...
    public void execute() {
        ...
        spawnMonsterData.setLocationId(LocationId.SPACE_21);
        spawnMonsterData.setMonsterId(EpicMonsterId.TICK_TOCK_MEN);
        ...
        private void triggerFailureEffect() {
            ServicePlatform.get().getService().addEventCommand(in -> {
                ...
                for (ClueInfo spawnedClue : ServicePlatform.get().getCluePool().getSpawnedClues()) {
                    ServicePlatform.get().getTokenService().discardClue(spawnedClue.getCurrentLocationId());
                }
    ...
    private class BeforeDefeatMonsterListener extends EventListenerImpl<DefeatMonsterData> {
        ...
        public void onNotify(DefeatMonsterData eventData) {
            if (eventData.getMonsterInfo().getMonsterId() != EpicMonsterId.TICK_TOCK_MEN) {
                return;
            }
            ServicePlatform.get().getService().hold();
