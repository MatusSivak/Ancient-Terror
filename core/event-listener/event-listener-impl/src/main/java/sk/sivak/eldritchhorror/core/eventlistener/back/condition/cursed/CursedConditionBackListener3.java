...
public class CursedConditionBackListener3 extends AbstractCursedConditionBackListener {

    public CursedConditionBackListener3(ConditionInfo conditionInfo) {
        super(conditionInfo);
    }
    ...
    protected void execute() {
        ...
        List<InvestigatorId> notCursedInvestigators = new LinkedList<>();
        for (InvestigatorRead investigator : investigators) {
            ...
            if (ServicePlatform.get().getConditionsDeck().hasCondition(investigatorId, ConditionId.CURSED)) {
                continue;
            }
            notCursedInvestigators.add(investigatorId);
        }


        TypewriterUtils.confirmInfos("[#BAD]Another investigator of your choice\n" +
                "that does not have a Cursed Condition\n" +
                "gains a Cursed Condition[]").subscribe(() -> {
            if (notCursedInvestigators.isEmpty()) {
                TypewriterUtils.confirmInfos("Everyone has Cursed Condition.").subscribe(() -> {
            ...
            investigatorRestriction.addAllowedInvestigators(notCursedInvestigators);
