...
import sk.sivak.eldritchhorror.core.constants.asset.MysteriousTomeAsset;
 ...
 * @author msivak
 */
public class MysteriousTomeListener extends AbstractAssetListener<MysteriousTomeAsset> {

    private static final Logger logger = LogManager.getLogger(MysteriousTomeListener.class);
    ...
    private class BeforeRerollUsingFocus extends AbstractAssetEventListener<RerollUsingData> {
        ...
        public BeforeRerollUsingFocus() {
            super(MysteriousTomeListener.this);
        }
        ...
        protected Runnable getEventAction(RerollUsingData input) {
            return () -> {
                ...
                if (input.getTestData().getStat() != Stat.WILL && input.getTestData().getStat() != Stat.LORE) {
                    wantsToUseSpecialAsked = false;
                    return;
                }

                Question question = new Question();
                question.setTitle("Reroll two dice using Mysterious Tome?");
