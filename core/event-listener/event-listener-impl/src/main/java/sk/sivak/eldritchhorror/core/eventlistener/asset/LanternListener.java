...
import sk.sivak.eldritchhorror.core.constants.asset.LanternAsset;
 ...
 * @author msivak
 */
public class LanternListener extends AbstractAssetListener<LanternAsset> {

    private static final Logger logger = LogManager.getLogger(LanternListener.class);
    ...
    private class RegisterUsableAssetListener extends AbstractAssetEventListener<TestData> {
        ...
        protected Runnable getEventAction(TestData input) {
            return () -> {
                ...
                UsableAsset usableAsset = addUsableAsset(input, getAssetInfo(), 0, 1);
        ...
        private Action0 getOnUseAction(TestData input) {
            return () -> {
                ...
                request.setTitle("Lantern used");
                request.setDisplayAssetResponseType(DisplayAssetResponseType.NOTHING);
                ServicePlatform.get().getGameService().showCard(request)
                        .subscribe(showCardResponse -> onUse(showCardResponse, input));
            };
        }

        void onUse(ShowCardResponse showCardResponse, TestData input) {
