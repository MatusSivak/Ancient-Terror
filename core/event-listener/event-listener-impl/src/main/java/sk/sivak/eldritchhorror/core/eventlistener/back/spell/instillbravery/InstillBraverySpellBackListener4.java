...
public class InstillBraverySpellBackListener4 extends AbstractSpellBackListener {

    public InstillBraverySpellBackListener4(SpellInfo spellInfo) {
        super(spellInfo);
    }
    ...
    private void on1() {
        TypewriterUtils.confirmInfos("[#BAD]That investigator loses 1 Health[]").subscribe(() -> {
            ServicePlatform.get().getService().hold();
            ServicePlatform.get().getEncounterService().finishTypewriterPaper();
    ...
    private void on2() {
        TypewriterUtils.confirmInfos("[#GOOD]Prevent that investigator from losing up to 4 Sanity instead.[]").subscribe(() -> {
