...
import rx.Single;
import sk.sivak.eldritchhorror.core.constants.artifact.AlienDeviceArtifact;
...
import sk.sivak.eldritchhorror.core.eventlistener.asset.AxeListener;
...
public class AlienDeviceListener extends AbstractArtifactListener<AlienDeviceArtifact> {

    private static final Logger logger = LogManager.getLogger(AlienDeviceListener.class);
    ...
    private class RegisterUsableAssetListener extends AbstractArtifactEventListener<TestData> {
        ...
        public RegisterUsableAssetListener() {
            super(AlienDeviceListener.this);
        }
    ...
    private class RerollUsingAssetsListener extends AbstractArtifactEventListener<TestData> {
        ...
        private void onSpend(SpendData spendData, TestData input) {
            if (spendData.hasEnough()) {
                ...
                ServicePlatform.get().getGameService().showCard(showCardRequest).subscribe(showCardResponse -> notEnoughSanity(input));
            }
        }

        private void notEnoughSanity(TestData input) {
            ServicePlatform.get().getService().convertTo(TestData.class, () -> input);
        }
