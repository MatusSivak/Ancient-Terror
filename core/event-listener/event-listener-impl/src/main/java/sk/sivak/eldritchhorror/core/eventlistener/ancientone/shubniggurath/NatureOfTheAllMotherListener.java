...
public class NatureOfTheAllMotherListener extends AbstractMysteryListener {


    private int progress;
    private AfterGainClueListener afterGainClueListener;

    public NatureOfTheAllMotherListener(MysteryCardInfo mysteryCardInfo) {
    ...
    public void register() {
        ...
        for (ClueInfo spawnedClue : ServicePlatform.get().getCluePool().getSpawnedClues()) {
            ...
            if (clueLocationInfo.getLocationType() == LocationType.WILDERNESS) {
                continue;
            }
            FindNearestData nearestSeaSpace = ServicePlatform.get().getLocationMap().findNearest(spawnedClue.getCurrentLocationId(),
                    location -> location.getLocationType() == LocationType.WILDERNESS);
