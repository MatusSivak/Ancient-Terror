...
import sk.sivak.eldritchhorror.core.constants.asset.AgentOfSecretsAsset;
 ...
 * @author msivak
 */
public class AgentOfSecretsListener extends AbstractAssetListener<AgentOfSecretsAsset> {

    private static final Logger logger = LogManager.getLogger(AgentOfSecretsListener.class);
    ...
    private class RegisterUsableAssetListener extends AbstractAssetEventListener<TestData> {
        ...
        public RegisterUsableAssetListener() {
            super(AgentOfSecretsListener.this);
        }

        @Override
        protected Runnable getEventAction(TestData input) {
            return () -> {
                if (input.getStat() != Stat.LORE && input.getStat() != Stat.OBSERVATION) {
                    return;
                }
        ...
        protected Runnable getEventAction(TestData input) {
            return () -> {
                ...
                ServicePlatform.get().getGameService().showCard(showCardRequest)
                        .subscribe(showCardResponse -> onAnswer(input, showCardResponse));
            };
        }

        private void onAnswer(TestData input, ShowCardResponse showCardResponse) {
