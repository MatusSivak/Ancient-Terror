...
public class VoiceOfRaSpellBackListener1 extends AbstractVoiceOfRaSpellBackListener {

    public VoiceOfRaSpellBackListener1(SpellInfo spellInfo) {
        super(spellInfo);
    }
    ...
    private void on0() {
        ...
        ServicePlatform.get().getEncounterService().displayButtons(
                ...
                "[#BAD]Lose one Sanity[]\n[#BAD]Gain a Madness Condition[]").subscribe(choice -> {
    ...
    private void on2() {
        ServicePlatform.get().getTokenService().canSpend(0,0,0,1).subscribe(canSpend -> {
            if (!canSpend.hasEnough()) {
                ...
                TypewriterUtils.noYesQuestion("[#BAD]Spend one Sanity[] to\n[#GOOD]Recover one Health[]?", () -> {
                    ServicePlatform.get().getService().hold();
                    ServicePlatform.get().getEncounterService().finishTypewriterPaper();
                    ServicePlatform.get().getService().release();
                }, () -> {
                    ServicePlatform.get().getTokenService().spend(0,0,0,1).subscribe(spend -> {
                        ServicePlatform.get().getService().hold();
                        ServicePlatform.get().getEncounterService().finishTypewriterPaper();
                        if (spend.hasEnough()) {
                            spend.pay();
