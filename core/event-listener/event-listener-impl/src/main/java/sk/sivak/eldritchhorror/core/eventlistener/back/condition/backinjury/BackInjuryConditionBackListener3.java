...
public class BackInjuryConditionBackListener3 extends AbstractConditionBackListener {

    public BackInjuryConditionBackListener3(ConditionInfo conditionInfo) {
        super(conditionInfo);
    }
    ...
    protected void execute() {
        TypewriterUtils.confirmInfos(
                "[#BAD]Discard all but one\nof your Item possessions[]",
                "[#GOOD]Discard this card[]").subscribe(() -> {
            ServicePlatform.get().getService().hold();
            ServicePlatform.get().getEncounterService().finishTypewriterPaper();
            List<? extends CardInfo> investigatorItems = findInvestigatorItems();
            if (investigatorItems.isEmpty() || investigatorItems.size() == 1) {
                discardThisCard();
            } else {
                for (int i = 0; i < investigatorItems.size() - 1; i++) {
                    ServicePlatform.get().getService().addEventCommand(whatever -> {
                        selectItemToDiscard(findInvestigatorItems());
                    });
                }
                discardThisCard();
            }
