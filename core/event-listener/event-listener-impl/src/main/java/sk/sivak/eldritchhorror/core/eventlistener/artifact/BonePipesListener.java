...
import rx.Single;
import sk.sivak.eldritchhorror.core.constants.artifact.BonePipesArtifact;
...
public class BonePipesListener extends AbstractArtifactListener<BonePipesArtifact> {
    ...
    private class InitActionButtonListener extends EventListenerImpl<Void> {
        ...
        public String toString() {
            return "BonePipesListener - InitActionButton";
        }
    }
    ...
    private class ActionListener extends AbstractActionPhaseListener<ActionListener.BonePipesAction> {
        ...
        protected String getTexturePath() {
            return "card/artifact/BONE_PIPES.jpg";
        }
        ...
        @Override
        protected BonePipesAction createAction() {
            return new BonePipesAction();
        }
        ...
        protected class BonePipesAction extends AbstractActionPhaseAction {
            ...
            private void onCanSpend(SpendData spendData) {
                ...
                showCardRequest.setTitle("Test Lore to move or damage Monster.");
            ...
            private void afterTest(TestData testData) {
                ...
                question.setTitle("Spend 1 Sanity to move or damage Monster?");
            ...
            private void moveOrDamageMonster() {
            ...
            private void onMonsterSelect(MonsterInfo monsterInfo) {
                if (monsterInfo.isEpic()) {
                    ...
                    Question<Boolean> question = new Question<>();
                    question.setTitle("Move Monster?");
                    question.setOptions(Arrays.asList(
                            new Question.Option<>("No", false, 0x800000ff),
                            new Question.Option<>("Yes", true, 0x008000ff)
                    ));
                    ServicePlatform.get().getGameService().ask(question).subscribe(answer -> onMoveMonsterAnswer(answer, monsterInfo));
                }
            }

            private void onMoveMonsterAnswer(Answer<Boolean, Object> answer, MonsterInfo monsterInfo) {
                if (answer.getResponseData()) {
                    moveMonster(monsterInfo);
                } else {
                    dealDamageToMonster(monsterInfo);
                }
            }

            private void moveMonster(MonsterInfo monsterInfo) {
                ...
                List<LocationId> availableLocations = new LinkedList<>();
                for (LocationInfo.Connection connection : connections) {
                    availableLocations.add(connection.getLocationId());
                }
                SelectLocationData selectLocationData = new SelectLocationData(availableLocations);
